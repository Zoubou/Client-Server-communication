...
def run_server():
    ...
    while True:
        ...
        clientConnection, clientAddress = serverTCPSocket.accept()
        print(f"New connection from client: {clientAddress}")

        clientMessage = clientConnection.recv(1024).decode()
        if clientMessage == "Hello Server, update this array":
            print(f"Received from client: {clientMessage}")

            serverHelloMessage = "Hello Client, here is the updated array"
            clientConnection.send(serverHelloMessage.encode())

            clientRequestMessage = clientConnection.recv(1024).decode()
            serialized_data = clientConnection.recv(131072)
            ...
            if clientRequestMessage == "Give me the updated array":
                ...
                if solutionMethod[0] == 1:
                    ...
                    result = print(bcolors.HEADER + "Program termination requested. Exiting..." + bcolors.ENDC)

                serialized_updated_data = pickle.dumps(result)
                exec_Time = pickle.dumps(exec_Time)

                clientConnection.send(serialized_updated_data)
                clientConnection.send(exec_Time)
                print(f"Sent updated array to client")
            else:
                print("Received an unrecognized message from client.")
        else:
            print("Received an unrecognized message from client.")
